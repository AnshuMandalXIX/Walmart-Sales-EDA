...
# type_labels = ['Walmart Supercenter', 'Walmart Discount Store', 'Walmart Neighborhood Market']
# type_s = st.sidebar.multiselect("Choose type of Sales", types)
# if not type_s:
#     df2 = df.copy()
# else:
#     type_numbers = [type_labels.index(label) for label in type_s]
#     df2 = df[df["Type"].isin(type_labels)]

# Create for Month

# mon_s = st.sidebar.multiselect("Choose Month of Sales", sorted(df2["month"].unique()))
# if not mon_s:
#     df3 = df2.copy()
# else:
